...
export const getWeekForecastWeather = (response, descriptions_list, temperatureUnit = 'Celsius') => {
  let foreacast_data = [];
  ...
  forecast_keys.forEach((key, idx) => {
    ...
    dayAvgsList.push({
      ...
      temp: temperatureUnit === 'Fahrenheit'
        ? getAverage(dayTempsList.map(temp => (temp * 9) / 5 + 32))
        : getAverage(dayTempsList),
      humidity: getAverage(grouped_forecast_data[key].map(item => item.humidity)),
      wind: getAverage(grouped_forecast_data[key].map(item => item.wind), false),
      clouds: getAverage(grouped_forecast_data[key].map(item => item.clouds)),
